...
This guide aims to target people who are
* Proficient in the Rust programming language
* At least a bit familiar with the basics of low level programming (at least with how pointers work)
* Want to work with low level modern GPU programming

But this guide assumes no knowledge with how graphics APIs or GPUs work in general and aims to fill you in from scratch to use the Vulkan API.
...
One of the features we use, [descriptor heaps](https://docs.vulkan.org/features/latest/features/proposals/VK_EXT_descriptor_heap.html) is not part of the road map. But it officially deprecates the old method and is expected to become part of a future road map or Vulkan version once it's been tested enough. 

As of writing this article, it is available on any NVIDIA GPU that supports driver 595.79 or newer which means any GPU Turing+. AMD is taking a bit longer to implement this extension.
{{% /expand %}}
